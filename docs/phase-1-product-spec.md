...
2026-08-15 最新決策：先交付一個管理者本人可使用、可穩定運行的版本。本文後續完整第一階段規格仍保留作為未來方向，但在使用者再次明確要求前，不繼續實作尚未完成的產品化功能。

目前 MVP 範圍：

- 單一使用者透過 `ALLOWED_USERS` 使用 Bot。
- 保留現有 `/start`、`/help`、`/status`、`/update_live`、`/add_zin`、`/list_zin`、`/query_zin`。
- Live 使用完整文字歌單更新。
- Live 保存輸入順序。
- 第一首顯示為暖身，最後一首顯示為收操。
- 純重新排序不新增或結束使用歷史。
...
- 教材詳情以粗體歌曲名稱、使用狀態與使用日期／歷史呈現，不顯示歌手、BPM、時長或曲風，並提供快速切換教材類型的按鈕。
...
- 草稿、預覽及最終確認。
- 個別歌曲異動日期與歷史修正。
- `users`、`ADMIN_USERS`、Admin／Instructor 角色。
- 歌曲別名及模糊匹配人工確認。
- Telegram 圖片 OCR。
- Web、Notion、公開註冊、廣告與訂閱。

注意：目前 `/update_live` 和 `/add_zin` 仍會在成功解析後立即寫入資料庫。單人 MVP 接受此行為，未來進入多人或正式產品階段時，才改成草稿與確認流程。
...
### 2.2 Stakeholder 訪談確認的輸入來源

2026-08 stakeholder 訪談後確認：

- Live 歌單會直接提供文字，而且文字行序就是實際播放順序。
- ZIN 與 Mega Mix（簡稱 MM）教材會提供照片。
- 第一階段不讓 Telegram Bot 自動辨識照片，避免 Vision／OCR API 的用量費用與額外複雜度。
- 管理者先把教材照片交給 Codex，由 Codex 逐張辨識、合併重疊內容、去重，再輸出一行一首、程式可讀的固定文字格式。
- 管理者將整理後的文字貼入 Telegram Bot，經預覽與確認後建立教材。

教材照片的人工流程：

1. 管理者把 ZIN／MM 教材照片貼到 Codex。
2. Codex 協助 OCR、依照片順序合併、去重並輸出固定文字格式。
3. 管理者檢查後，把文字複製到 Telegram Bot。
4. Bot 解析、比較、顯示異常與預覽。
5. 人工最終確認後才修改資料庫。

固定辨識格式請參考 [`docs/program-image-recognition-format.md`](program-image-recognition-format.md)。每次實際辨識結果存放於 `imports/`，例如 `imports/mm-114.md`。
...
例如 `ZIN + 124` 與 `MM + 124` 可以同時存在，但不能存在兩筆 `MM + 114`。Stakeholder 確認所有教材都有期數，而照片不一定顯示發行月份，因此 `release_month` 為選填資訊，不作為 MVP 的唯一鍵。
...
Stakeholder 訪談確認 Live 歌單不是無序集合。歌曲位置具有業務意義，例如暖身歌曲通常位於前段、收操歌曲位於後段。

規則：

- Live 文字輸入每行一首，行序就是更新後的播放順序。
- Bot 顯示目前 Live 歌單時必須按照保存的順序排列。
- 新增、移除與順序調整是三種不同差異。
- 歌曲仍在 Live、但位置改變時，只更新位置，不結束或重建使用歷史。
- 同一份 Live 歌單中同一首歌曲只允許出現一次；重複輸入列為異常，要求使用者修正。
- 第一階段只保存目前 Live 歌單順序，不建立每日順序快照。
- 若未來需要查詢歷史上某一天的完整播放順序，再新增 playlist snapshot 模型，不在第一階段處理。

建議在 `live_history` 的 active 紀錄增加：

```text
position            從 1 開始的目前 Live 位置
```

每次完整 Live 更新確認後，Transaction 內必須同步更新所有 active 歌曲的 `position`。查詢目前歌單使用 `ORDER BY position`，不能再用加入日期作為主要排序。
...
Live 由 stakeholder 直接提供文字。每行一首，代表「更新後的完整 Live 歌單」，行序同時代表播放順序：
...
這不是只列新增或移除的差異；Bot 必須用完整清單和目前 active 歌單比較，並保存文字中的順序。
...
- 目前有、新清單沒有：待移除。
- 兩邊都有：保持使用中。
- 兩邊都有但位置不同：待更新順序，不重建 Live 歷史。
- 模糊匹配：要求人工確認。
- 已在使用卻被判定新增：顯示異常，不重複建立 active 歷史。
- 不在使用卻被判定移除：顯示異常，不建立假的歷史。
- 同一首歌在輸入中重複：顯示異常，確認前必須移除重複項目。

移除必須清楚列出，因為它會結束目前 active 的使用區間。

Live 預覽與 `/status` 應顯示編號順序。最終摘要至少包含新增、移除、保持使用中及位置調整數量。
...
- Bot 內的教材照片 OCR；教材照片仍由 Codex 人工整理成文字。
...
- 教材詳情使用粗體歌曲名稱、使用狀態與使用日期／歷史；不顯示歌手、BPM、時長或曲風，並提供 ZIN／Mega Mix 快速瀏覽按鈕。
- `ALLOWED_USERS` 白名單。
...
- Go Mutex 序列化寫入。
- Live 多段使用歷史。
- Live 會保存輸入行序，`/status` 依序顯示，並標示第一首暖身、最後一首收操。
- 單純重新排序不會建立或結束 Live 歷史；重複歌曲會被拒絕。
- 舊 SQLite 資料庫啟動時會自動補上並回填 active Live 的 `position`。
- ZIN 教材重新匯入覆蓋。
- Parser 與 DB 自動化測試。
- 完整 README（已在 Git commit `d05adf5 update readme`）。

目前 SQLite schema 包含既有 ZIN 表與新的通用教材表：
...
- [x] 通用 `program_releases`／`program_tracks` schema。
- [ ] 現有 ZIN 資料 migration。
- [x] MM 教材支援。
- [x] 歌手、BPM、時長、分類欄位。
- [ ] 正式 `users` 資料表。
...
- [ ] `song_aliases`。
- [ ] 取消 95% 自動模糊合併。
- [x] `program-import-v1` 固定教材文字格式 parser。
- [x] 教材按類型＋期數查詢。
- [ ] 教材完整清單差異預覽。
- [ ] 教材關聯移除確認。
- [ ] `import_sessions` 草稿。
...
9. 實作 Live 草稿、差異計算、順序保存、逐筆日期與歷史驗證。
...
- [ ] 移除教材關聯不刪歌曲或 Live 歷史。
- [ ] Live 完整清單差異正確。
- [ ] Live 文字行序能正確保存並由 `/status` 依序顯示。
- [ ] 只調整順序不會建立或結束 Live 歷史。
- [ ] Live 重複歌曲會被阻止並要求修正。
- [ ] 完全一致自動匹配。
- [ ] 模糊匹配永遠等待人工確認。
- [ ] 個人別名不影響其他使用者。
...
6. Live 文字的行序會被保存，`/status` 按相同順序列出暖身到收操歌曲。
7. 只調整既有歌曲順序時，不會新增或結束使用歷史。
8. 老師可為所有異動指定預設日期，再修改特定歌曲日期。
9. 模糊歌名必須人工選擇，不能自動合併。
10. 未確認前資料庫完全不變。
11. Bot 重啟後可以繼續未過期草稿。
12. 查詢 ZIN／MM 時能顯示該老師對教材歌曲的所有使用歷史。
13. 現有資料在 migration 後仍可查詢且筆數合理。
14. `go test ./...` 全部通過。
